...
// We can pass global values from C++ to shaders using the constant buffer below - it must match the
// similar C++ struct in common.h. However, there is no immediate need to use this in the assignment as
// each particle carries all the data it needs. However, if you see the need for some settings that globally
// control all firework particles you could add that here, in common.h and pass over from C++ in Scene.cpp
