//--------------------------------------------------------------------------------------
// Pixel shader for textured polgyons tinted a per-primitive colour
//--------------------------------------------------------------------------------------
// Pixel shader samples a diffuse texture map and tints with a colour sent from earlier shader
...
float4 main(ColourTexturePixelShaderInput input) : SV_Target
{
    ...
    // Blend texture colour with per-primitive colour
    float3 finalColour = input.colour.rgb * diffuseMapColour * input.colour.a; // Use alpha on additive blended particle
